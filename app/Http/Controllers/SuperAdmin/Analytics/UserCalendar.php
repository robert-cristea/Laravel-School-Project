...
class UserCalendar extends Controller 
{
    ...
    ////////////////////////////////// User Calender /////////////////////////////
    // Engagement
    public function showEngageUser(Request $request){
        $dates = $request->date;
        $dates = explode('-', $dates);
        $start_date = $dates[0];
        ...
        $user_views = Video_likes::whereBetween('date', [$start_date, $end_date])
                                ->where('user_id', $first_user_id)->count();
        $usage_time = 0;
        $user_likes = Video_likes::whereBetween('date', [$start_date, $end_date])
                                ->where('user_id', $first_user_id)->where('likes', 1)->count();
    ...
    // Weekly
    public function showWeeklyUser(Request $request){
        $dates = $request->date;
        $dates = explode('-', $dates);
        $start_date = $dates[0];
        $start_date = explode('/', $start_date);
        ...
        $week_progress = Week_progress::where('user_id', $first_user_id)
    ...
    // Monthly
    public function showMonthlyUser(Request $request){
        $dates = $request->date;
        $dates = explode('-', $dates);
        $start_date = $dates[0];
        $start_date = explode('/', $start_date);
        ...
        $all_checks = All_checks::where('user_id', $first_user_id)
    ...
    // Coaching
    public function showCoachingUser(Request $request){
        $dates = $request->date;
        $dates = explode('-', $dates);
        $start_date = $dates[0];
        $start_date = explode('/', $start_date);
        ...
        $user_ave_rates = Coaching_reports::whereBetween('report_date', [$start_date, $end_date])
                            ->where('user_id', $first_user_id)->get();
        

        $user_depress = Coaching_reports::whereBetween('report_date', [$start_date, $end_date])->where('user_id', $first_user_id)->where('motif_seance_id', 1)->count();
        if ($user_depress != 0) {
            $user_depress = Coaching_reports::whereBetween('report_date', [$start_date, $end_date])->where('user_id', $first_user_id)->where('motif_seance_id', 1)->count()/$user_coaching*100;
        } else {
            $user_depress = 0;
        }
        $user_parenting = Coaching_reports::whereBetween('report_date', [$start_date, $end_date])->where('user_id', $first_user_id)->where('motif_seance_id', 2)->count();
        if ($user_parenting !=0) {
            $user_parenting = Coaching_reports::whereBetween('report_date', [$start_date, $end_date])->where('user_id', $first_user_id)->where('motif_seance_id', 2)->count()/$user_coaching*100;
        } else {
            $user_parenting = 0;
        }
        $user_relation = Coaching_reports::whereBetween('report_date', [$start_date, $end_date])->where('user_id', $first_user_id)->where('motif_seance_id', 3)->count();
        if ($user_relation != 0) {
            $user_relation = Coaching_reports::whereBetween('report_date', [$start_date, $end_date])->where('user_id', $first_user_id)->where('motif_seance_id', 3)->count()/$user_coaching*100;    
        } else {
            $user_relation = 0;
        }
        $user_mouring = Coaching_reports::whereBetween('report_date', [$start_date, $end_date])->where('user_id', $first_user_id)->where('motif_seance_id', 4)->count();
        if ($user_mouring != 0) {
            $user_mouring = Coaching_reports::whereBetween('report_date', [$start_date, $end_date])->where('user_id', $first_user_id)->where('motif_seance_id', 4)->count()/$user_coaching*100;
        } else {
            $user_mouring = 0;
        }
        $user_conflict = Coaching_reports::whereBetween('report_date', [$start_date, $end_date])->where('user_id', $first_user_id)->where('motif_seance_id', 5)->count();
        if ($user_conflict != 0) {
            $user_conflict = Coaching_reports::whereBetween('report_date', [$start_date, $end_date])->where('user_id', $first_user_id)->where('motif_seance_id', 5)->count()/$user_coaching*100;    
        } else {
            $user_conflict = 0;
        }
        $user_confidence = Coaching_reports::whereBetween('report_date', [$start_date, $end_date])->where('user_id', $first_user_id)->where('motif_seance_id', 6)->count();
        if ($user_confidence != 0) {
            $user_confidence = Coaching_reports::whereBetween('report_date', [$start_date, $end_date])->where('user_id', $first_user_id)->where('motif_seance_id', 6)->count()/$user_coaching*100;    
        } else {
            $user_confidence = 0;
        }
        $user_addiction = Coaching_reports::whereBetween('report_date', [$start_date, $end_date])->where('user_id', $first_user_id)->where('motif_seance_id', 7)->count();
        if ($user_addiction != 0) {
            $user_addiction = Coaching_reports::whereBetween('report_date', [$start_date, $end_date])->where('user_id', $first_user_id)->where('motif_seance_id', 7)->count()/$user_coaching*100;    
        } else {
            $user_addiction = 0;
        }
        $user_others = Coaching_reports::whereBetween('report_date', [$start_date, $end_date])->where('user_id', $first_user_id)->where('motif_seance_id', 8)->count();
        if ($user_others != 0) {
            $user_others = Coaching_reports::whereBetween('report_date', [$start_date, $end_date])->where('user_id', $first_user_id)->where('motif_seance_id', 8)->count()/$user_coaching*100;    
        } else {
            $user_others = 0;
        }
