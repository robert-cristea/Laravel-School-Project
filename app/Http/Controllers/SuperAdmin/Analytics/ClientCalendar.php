...
class ClientCalendar extends Controller
{
    /**
    ...
    ////////////////////////////////// Client Calender /////////////////////////////
    // Engagement
    public function showEngageClient(Request $request)
    {
        $dates = $request->date;
        $dates = explode('-', $dates);
        $start_date = $dates[0];
        ...
        $cor_video_views = Video_likes::Join('vieva_team_members', 'vieva_team_members.user_id', '=', 'vieva_video_likes.user_id')
            ->whereBetween('vieva_video_likes.date', [$start_date, $end_date])
        ...
        $cor_video_comments = Videos_comments::Join('vieva_team_members', 'vieva_team_members.user_id', '=', 'vieva_video_comments.user_id')
            ->whereBetween('vieva_video_comments.date', [$start_date, $end_date])
            ->where('corporate_client_id', $first_client_id)
            ->count();
        $cor_video_challenges = Users_challenges::Join('vieva_team_members', 'vieva_team_members.user_id', '=', 'vieva_users_challenges.user_id')
            ->whereBetween('vieva_users_challenges.date', [$start_date, $end_date])
            ->where('corporate_client_id', $first_client_id)
            ->where('accepted', 1)
            ->count();
        $cor_quotes = Quote_likes::whereBetween('creation_date', [$start_date, $end_date])->count();
        $output = '<tbody>
                        <tr>
    ...
    // Weekly
    public function showWeeklyClient(Request $request)
    {
        $dates = $request->date;
        $dates = explode('-', $dates);
        $start_date = $dates[0];
        ...
        $output .= (round($week_progress->average("workload_level"), 2) +
            round($week_progress->average("stress_level"), 2) +
            round($week_progress->average("energy_level"), 2)) / 3;
        $output .= '/5
                            </td>
                        </tr>
                        <tr>
    ...
    // Monthly
    public function showMonthlyClient(Request $request)
    {
        $dates = $request->date;
        $dates = explode('-', $dates);
        $start_date = $dates[0];
        ...
        $all_checks = All_checks::Join('vieva_team_members', 'vieva_team_members.user_id', 'vieva_all_checks.user_id')
            ->where('corporate_client_id', $first_client_id)
            ->whereBetween('vieva_all_checks.check_datetime', [$start_date, $end_date])
            ->where('checks_type', 1)
            ->get();
    ...
    // Populateion
    public function showPopulationClient(Request $request)
    {
        $dates = $request->date;
        $dates = explode('-', $dates);
        $start_date = $dates[0];
    ...
    // Coaching
    public function showCoachingClient(Request $request)
    {
        $dates = $request->date;
        $dates = explode('-', $dates);
        $start_date = $dates[0];
        ...
        $cor_coaching_session = Coaching_reports::Join('vieva_team_members', 'vieva_team_members.user_id', '=', 'vieva_coaching_reports.user_id')
            ->whereBetween('vieva_coaching_reports.report_date', [$start_date, $end_date])
            ->where('corporate_client_id', $first_client_id)
            ->count();
                            ...
                            <table class="table table-bordered">
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>';
        $output .= round($cor_ave_coaching->average("rating"), 2);
        $output .= ' stars
