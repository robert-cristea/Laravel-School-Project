...
class UserController extends Controller
{
    public function weekly_check(Request $request)
    {
        ...
        $workload = Week_progress::where('user_id', $first_user_id)
            ->whereBetween('date', [$start_date, $end_date])->pluck('workload_level');
        $stress = Week_progress::where('user_id', $first_user_id)
            ->whereBetween('date', [$start_date, $end_date])->pluck('stress_level');
        $energy = Week_progress::where('user_id', $first_user_id)
            ->whereBetween('date', [$start_date, $end_date])->pluck('energy_level');
