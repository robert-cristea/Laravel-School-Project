...
class ClientController extends Controller
{
    public function weekly_check(Request $request)
    {
        ...
        $week_progress = Week_progress::Join('vieva_team_members', 'vieva_team_members.user_id', '=', 'vieva_week_progress.user_id')
            ->where('vieva_team_members.corporate_client_id', $client_id)
            ->whereBetween('vieva_week_progress.date', [$start_date, $end_date])
            ->get();
        // return response(['week' => $week_progress, 'client' => $request->client_id]);
        // $labels = Week_progress::whereBetween('date', [$start_date, $end_date])->pluck('date');
        $labels = Arr::pluck($week_progress, 'ymd');
        $workload = Week_progress::Join('vieva_team_members', 'vieva_team_members.user_id', '=', 'vieva_week_progress.user_id')
            ->where('corporate_client_id', $client_id)
            ->whereBetween('vieva_week_progress.date', [$start_date, $end_date])->pluck('workload_level');
        $stress = Week_progress::Join('vieva_team_members', 'vieva_team_members.user_id', '=', 'vieva_week_progress.user_id')
            ->where('corporate_client_id', $client_id)
            ->whereBetween('vieva_week_progress.date', [$start_date, $end_date])->pluck('stress_level');
        $energy = Week_progress::Join('vieva_team_members', 'vieva_team_members.user_id', '=', 'vieva_week_progress.user_id')
            ->where('corporate_client_id', $client_id)
            ->whereBetween('vieva_week_progress.date', [$start_date, $end_date])->pluck('energy_level');
    ...
    public function monthly_check(Request $request)
    {
        ...
        $end_y = $end_date[2];
        $end_date = implode('-', array($end_y, $end_m, $end_d));
        $first_client_id = $request->client_id;
