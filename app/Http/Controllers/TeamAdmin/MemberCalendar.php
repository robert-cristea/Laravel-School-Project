...
class MemberCalendar extends Controller 
{
    /**
    ...
    ////////////////////////////////// Per Member Calender /////////////////////////////
    // Engagement
    public function showMemberEngage(Request $request){
        $id = Auth::user()->id;
        $first_member_id = Team_members::where('corporate_group_admin_id', $id)->first()->user_id;
        ...
        $video_views = User_activities_videos::where('user_id', $first_member_id)
    ...
    // Weekly
    public function showMemberWeekly(Request $request){
        $id = Auth::user()->id;
        $first_member_id = Team_members::where('corporate_group_admin_id', $id)->first()->user_id;
        ...
        $week_progress = Week_progress::where('user_id', $first_member_id)
                    ->whereBetween('vieva_week_progress.date', [$start_date, $end_date])
                    ->get();
        $output = '<tbody>
                        <tr>
    ...
    // Monthly
    public function showMemberMonthly(Request $request){
        $id = Auth::user()->id;
        $first_member_id = Team_members::where('corporate_group_admin_id', $id)->first()->user_id;
        ...
        $all_checks = All_checks::where('user_id', $first_member_id)
                            ->whereBetween('vieva_all_checks.check_datetime', [$start_date, $end_date])->where('checks_type', 1)->get();
        $tatal_num = All_checks::where('user_id', $first_member_id)
    ...
    // Coaching
    public function showMemberCoaching(Request $request){
        $id = Auth::user()->id;
        $first_member_id = Team_members::where('corporate_group_admin_id', $id)->first()->user_id;
        ...
        $coaching_sessions = Coaching_reports::where('user_id', $first_member_id)
                ->whereBetween('report_date', [$start_date, $end_date])->count();
        $average_coaching_rates = Coaching_reports::where('user_id', $first_member_id)
                ->whereBetween('report_date', [$start_date, $end_date])->get();
        $total_users = Coaching_reports::where('user_id', $first_member_id)
                ->whereBetween('report_date', [$start_date, $end_date])->get()->count();
        $unique_users = Coaching_reports::where('user_id', $first_member_id)
                ->whereBetween('report_date', [$start_date, $end_date])->get()->groupBy('user_id')->count();
        ...
        $total_reasons = Coaching_reports::where('user_id', $first_member_id)
                    ->whereBetween('report_date', [$start_date, $end_date])->get()->count();
        if ($total_reasons != 0) {
            $depression = Coaching_reports::where('user_id', $first_member_id)
                    ->whereBetween('report_date', [$start_date, $end_date])->where('motif_seance_id', '1')->get()->count()/$total_reasons*100;
            $parenting_issues = Coaching_reports::where('user_id', $first_member_id)
                    ->whereBetween('report_date', [$start_date, $end_date])->where('motif_seance_id', '2')->get()->count()/$total_reasons*100;
            $relationship_issues = Coaching_reports::where('user_id', $first_member_id)
                    ->whereBetween('report_date', [$start_date, $end_date])->where('motif_seance_id', '3')->get()->count()/$total_reasons*100;
            $mourning = Coaching_reports::where('user_id', $first_member_id)
                    ->whereBetween('report_date', [$start_date, $end_date])->where('motif_seance_id', '4')->get()->count()/$total_reasons*100;
            $conflicts = Coaching_reports::where('user_id', $first_member_id)
                    ->whereBetween('report_date', [$start_date, $end_date])->where('motif_seance_id', '5')->get()->count()/$total_reasons*100;
            $self_confidence = Coaching_reports::where('user_id', $first_member_id)
                    ->whereBetween('report_date', [$start_date, $end_date])->where('motif_seance_id', '6')->get()->count()/$total_reasons*100;
            $addictions = Coaching_reports::where('user_id', $first_member_id)
                    ->whereBetween('report_date', [$start_date, $end_date])->where('motif_seance_id', '7')->get()->count()/$total_reasons*100;
            $others = Coaching_reports::where('user_id', $first_member_id)
                    ->whereBetween('report_date', [$start_date, $end_date])->where('motif_seance_id', '8')->get()->count()/$total_reasons*100;    
        } else {
            $depression = 0;
            $parenting_issues = 0;
